...
- 模块配置重新拆分：新增「集成」分组，钉钉拆成可单独安装的子模块（机器人对话、工作通知与提醒、日程与待办、审批与自动审批、个人数据、文档与表格），另新增「企业查询」「孤儿文件清理」模块；父模块关闭时子模块随之停用，子模块依赖未开启时开关置灰并提示原因。模块关闭后，对应工具、接口、后台任务与管理页面一并停用；「聊天平台机器人」不再连带控制钉钉。
- 状态监控新增「告警设置」（右上角）：可开关告警，选择通知渠道 —— 工作通知（按管理员角色或指定用户）、钉钉群机器人（Webhook 与加签密钥加密保存）、邮件，可分别发送测试；可设置告警规则、钉钉接口日调用量阈值、重复提醒间隔与恢复通知。
- 新增对外状态 API（供统一状态监控面板使用）：`/api/status/v1/health`、`/summary`、`/events`，令牌在「告警设置 → 状态 API」中生成（只显示一次），说明见 `docs/enterprise/status-api.md`。
- Cursor 模型按基础型号合并：同一型号的不同思考强度不再各算一个模型，改为在输入框选择思考强度（每个模型只显示它支持的档位与默认档）；Fast 版本单独列为一个模型，同样支持思考强度。旧的带强度后缀的模型 ID 在基础型号已发布时仍可使用。
- 新增《钉钉 AI 能力使用手册》（PDF，`docs/enterprise/handbook/`），按能力给出使用场景、范例提示词与预期结果。

#### 💄 Improvements

- 管理后台「IM 连接器」重新设计：按「连接凭据 → 机器人对话 → 通知应用 → 工作台能力 → 个人数据授权 → 已绑定用户」分组，说明文字改为问号提示，未保存修改时才显示底部保存栏；新增「确认卡片模板 ID」设置；CorpId 与环境变量中的确认卡片模板会预填显示（标注来源，未修改时不写入）。
- 界面与提示文案不再出现特定企业的叫法（如「服务号」统一为「通知应用」），引导管理员前往设置页的提示统一为「IM 连接器」。
- 状态监控：「近期任务」改为分页，新增「清除」（只隐藏已结束的任务记录，不删除数据）；去掉各区块下的长说明，必要的改为问号提示。

#### 🗃️ Database

- 新增迁移 0039：`platform_status_settings`（告警设置、任务清除时间、状态 API 令牌摘要）。

#### ⚠️ Notes

- 使用 `minimal` / `standard` 预设部署时，新增的钉钉、企业查询模块属于完整配置，升级后默认不启用；使用默认 `full` 预设或未设置时行为不变。
- 管理员同步 Cursor 模型后，旧的带强度后缀的条目会合并到基础型号（已启用的旧条目会启用对应基础型号；仍被引用的旧条目保留）。
...
- 新增内置工具「钉钉文档与表格」：员工授权钉钉个人数据后，助手可以以本人身份搜索和阅读钉钉文档、浏览知识库、搜索和下载钉盘文件（Excel、PDF 等可直接分析）、读取在线表格与 AI 表格；支持追加文档内容、新建文档、向在线表格追加行、新增或修改 AI 表格记录，每次写入都要在确认卡上同意，卡片上显示实际要写入的内容（字段名而不是内部 id）。管理端新增「文档 / 钉盘 / 知识库」「在线表格 / AI 表格」两个开关（默认关闭，写入仍受「允许写操作」控制）。
- 一次处理多条：新增批量接口「完成多条待办」（钉钉个人数据、钉钉日程与待办）、「删除多条待办」「批量同意 / 拒绝审批」，要求处理多条时只调用一次、只弹一张确认卡；个别条目失败不影响其他条目，结果逐条列出成功或失败原因，缺权限时附申请链接。
- 同一轮里需要确认的多个操作合并处理：钉钉里只发一张确认卡（列出全部操作，一次批准或拒绝全部生效）；网页端确认栏新增「全部批准」「全部拒绝」，每项仍先经过各自的预览校验；确认栏与钉钉卡片的操作名称改为中文。
- 定期清理孤儿文件：每天 03:30 清理已无任何引用、且 7 天以上未被访问的存储文件（可用环境变量关闭或只试运行）。

#### ⚡ 钉钉接口调用量优化

- 访问令牌在所有进程间共享并按应用凭据（含密钥）隔离，并发刷新只请求一次；机器人客户端复用，不再每次发送都重新取令牌。
- 回复在 5 秒内完成时不再发送并撤回「正在思考…」占位消息。
- 通讯录同步：重启后不再立即全量同步（距上次成功不足 12 小时则顺延），查不到姓名触发的补同步冷却由 1 小时改为 6 小时，同一姓名 6 小时内不重复触发。
- 审批：待审批 / 我发起的 / 模板列表与审批详情改为跨进程共享缓存并合并并发请求，列表已含字段时不再逐条查详情；自动审批规则轮询由每 3 分钟改为每 10 分钟。
- 钉钉 API 调用计数覆盖全部调用路径（此前机器人发消息、卡片、登录等未计入）；新增每日调用量告警（`DINGTALK_API_DAILY_ALERT_THRESHOLD`，默认 5000 次，超过时通知平台管理员）。
- 「钉钉个人数据」群列表缓存 10 分钟、日志模板缓存 1 小时。

#### 🐛 Fixes

- 总结群聊时不再出现「钉钉接口返回错误（DINGTALK\_PERSONAL\_UPSTREAM）：下游缺少明确的集合字段，无法判定结果为空」：钉钉 CLI 在关键词搜不到消息时会报这个错，现视为「没有匹配的消息」；并引导助手按群名先找群再读群消息，不再把群名当成消息关键词搜索。
- 钉钉确认卡此前遇到多个待确认操作时只处理第一个、其余直接跳过；现在全部列出并按一次决定执行。
- 删除文件时可能误删仍被技能引用的存储文件（网页端与 OpenAPI 两条路径）。
- 确认卡预览失败时显示具体原因（如内容过大需分次写入）；缺少钉钉操作授权时确认卡上的「去授权」链接此前取不到。
