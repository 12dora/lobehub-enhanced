...
export const getEnterpriseLookupSettings = ({ ctx }: { ctx: SystemHandlerCtx }) =>
  executePlatformSystem(() => new EnterpriseLookupSettingsService(ctx.serverDB).get());
...
export const testEnterpriseLookupProviderHandler = ({
  ctx,
  input,
}: {
  ctx: SystemHandlerCtx;
  input: AdminSystemTestEnterpriseLookupProviderInput;
}) => executePlatformSystem(() => testEnterpriseLookupProvider(ctx.serverDB, input));
