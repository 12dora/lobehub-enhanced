...
10. **默认助理换模型即重置用户 inbox 行**：管理员保存 / 发布 / 回滚 `system_key=default-inbox` 且新版本 `dependency_snapshot.model` 的 `{providerKey, modelKey}` 相对上一已发布版本变化（或任一端没有 pair）时，同一事务内将所有用户 `agents.slug='inbox'` 且 `model`/`provider` 非空的行置空。遗留未迁移（session 形态、随机 slug）的 inbox 行不覆盖。新会话跟随新默认；已有会话与消息不动。观察模式下用户之后仍可再选模型。
...
- **[status-api.md](./status-api.md)** — 对外状态 API：健康检查、摘要、事件、令牌与 `schemaVersion`。
