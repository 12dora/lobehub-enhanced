...
- createReminder：recipients 直接写姓名（同名时写「姓名·部门」）或部门名，也可写 searchDirectory 返回的 staff:<id>/dept:<id>。title 为≤12字摘要（如 每日例会）。schedule.kind 为 once(需 date+time)/daily/weekly(weekdays 1-7 周一=1)/monthly(monthDays)；只填用到的字段，不要空字符串或空数组。时间按 Asia/Shanghai，当前时刻见返回的 serverNow。返回 needs_clarification 时列出候选「姓名 · 部门」请用户选择后用精确 token 重试；needs_confirmation 时先向用户确认，再以 confirmLargeAudience=true 重试。
