 ...
 * is due. Null / invalid `lastHeartbeatAt` means the task has never run — not
 * restart catch-up — so it is not kicked by the sweep. Lost-timer catch-up is
 * only `lastHeartbeatAt + interval <= now`.
 */
